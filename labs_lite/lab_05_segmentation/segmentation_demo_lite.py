#!/usr/bin/env python3
"""
Lab 5: Image Segmentation with UNet (LIGHTWEIGHT VERSION)
==========================================================

This is a lightweight version optimized for faster training on laptops.
Changes from original:
- Uses REAL Pet images from Oxford-IIIT Pet Dataset
- Training samples: 100 (from 800)
- Validation samples: 20 (from 200)
- Epochs: 10 (from 20)
- Image size: 64x64 (from 128x128)
- UNet channels: Reduced by 50% (32→64→128→256 vs 64→128→256→512)
...
class PetSegmentationDataset(Dataset):
    ...
    def __init__(self, data_dir, num_samples=100, img_size=64):
        ...
        print(f"Loaded {len(self.image_paths)} pet images")
...
class UNetLite(nn.Module):
    """Lightweight UNet architecture for semantic segmentation."""
    
    def __init__(self, in_channels=3, num_classes=2):
        super(UNetLite, self).__init__()
        
        # Encoder (downsampling) - reduced channels
...
def main():
    print("="*60)
    print("Lab 5 Lite: Image Segmentation with Real Pet Images")
    print("="*60)
    ...
    # Load datasets
    print("\nLoading Pet dataset...")
    train_dataset = PetSegmentationDataset(data_dir, num_samples=100, img_size=64)
    val_dataset = PetSegmentationDataset(data_dir, num_samples=20, img_size=64)
    ...
    # Train UNet
    print("\n" + "="*60)
    print("Training UNet Lite")
    print("="*60)
    unet = UNetLite(in_channels=3, num_classes=2)
    print(f"UNet parameters: {sum(p.numel() for p in unet.parameters()):,}")
    unet_history = train_model(unet, train_loader, val_loader, 10, device, "UNet Lite")
    ...
    cnn_history = train_model(cnn, train_loader, val_loader, 10, device, "Simple CNN")
    ...
    print("Final Results")
    print("="*60)
    print(f"UNet Lite - Final IoU: {unet_history['val_iou'][-1]:.4f}, "
    ...
    print("\nLab 5 Lite completed successfully!")
