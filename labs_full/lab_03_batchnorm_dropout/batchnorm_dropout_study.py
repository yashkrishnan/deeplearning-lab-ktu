...
Learning Objectives:
- Understand batch normalization and its benefits
- Learn how dropout prevents overfitting
- Compare training dynamics with different regularization techniques
- Analyze the impact on convergence speed and final accuracy
- Visualize the effects on training and validation curves
...
class BaselineNetwork(nn.Module):
    """
    Baseline neural network without any regularization.
    """
    def __init__(self):
...
class BatchNormNetwork(nn.Module):
    """
    Neural network with Batch Normalization after each layer.
    """
    def __init__(self):
...
class BatchNormDropoutNetwork(nn.Module):
    """
    Neural network with both Batch Normalization and Dropout.
    """
    def __init__(self, dropout_rate=0.5):
...
class RegularizationStudy:
    """
    Study the effects of batch normalization and dropout on neural networks.
    """
    ...
    def load_data(self):
        """
        Load CIFAR-10 dataset with train/validation split.
        ...
        train_indices = np.random.choice(len(trainset), 4000, replace=False)
        val_indices = np.random.choice(len(valset), 1000, replace=False)
    ...
    def train_model(self, model, train_loader, val_loader, model_name, num_epochs=15):
        """
    ...
    def run_study(self):
        ...
        """
        print("\n" + "="*60)
        print("  LAB 3: BATCH NORMALIZATION AND DROPOUT STUDY")
        print("="*60)
        ...
        for model, name in models:
            history, trained_model = self.train_model(
                model, train_loader, val_loader, name, num_epochs=15
            )
...
def main():
    ...
    """
    study = RegularizationStudy(output_dir='outputs')
    study.run_study()
    
    print("\n✨ Lab 3 completed! Check the 'outputs' folder for results.")
